        ...
        <transition name="fade-scale">
            <div class="relative z-10 max-w-md w-full backdrop-blur-sm p-8 rounded-2xl shadow-xl border border-gray-100"
                ...
                <p class="text-center text-gray-500 mb-6 text-sm">
                    مرحباً بعودتك. استمر في دعم عطاء وغُيّر حياة الآخرين
                </p>
                    ...
                    <div>
                        ...
                        <div class="relative">
                            ...
                            <button type="button" @click="showPassword = !showPassword"
                                class="absolute inset-y-0 right-3 flex items-center text-gray-500 hover:text-green-600 transition">
                                {{ showPassword ? '🙈' : '👁️' }}
                            </button>
                        </div>
                    </div>
...
const handleLogin = async () => {
    ...
    try {
        const response = await axios.post('/v1/users/login', form.value)
        ...
        if (response.data.status === 'success') {
            ...
            error.value = response.data.message || 'فشل تسجيل الدخول'
        }
    } catch (err) {
        error.value =
